...
var getRegisteredUser = async function (username, userOrg, registrationToken, isJson) {
	try {
		...
		if (user && user.isEnrolled()) {
			...
			logger.info(userOrg.toString());

			var usrOrg = userOrg.toString();
			var registerToken = registrationToken.toString();

			logger.info(usrOrg);
			logger.info(registerToken);

			if (usrOrg === 'Org2' && registerToken === 'org2token') {
			...
			logger.debug('Successfully enrolled username %s  and setUserContext on the client object', username);
				...
				throw new Error("User not enrolled: token mismatch");
			}
		}
		if (user && user.isEnrolled) {
